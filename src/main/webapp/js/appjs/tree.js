...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        ...
        if (dfop.showcheck) {
            for (var i = 0; i < 3; i++) {
                var im = new Image();
                im.src = dfop.cpath + dfop.cbiconpath + dfop.icons[i];
            }
        }
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if (deep == 1) {
                ht.push("<img class='bbit-tree-icon' src='" + dfop.cpath + "/images/s.gif'/>");
            }
            else if (deep > 1) {
                ht.push("<img class='bbit-tree-icon' src='" + dfop.cpath + "/images/s.gif'/>");
                for (var j = 1; j < deep; j++) {
                    ht.push("<img class='bbit-tree-elbow-line' src='" + dfop.cpath + "/images/s.gif'/>");
                }
            }
            ht.push("</span>");
            //img
            ...
            ht.push("<img class='bbit-tree-ec-icon ", cs.join(" "), "' src='" + dfop.cpath + "/images/s.gif'/>");
            ht.push("<img class='bbit-tree-node-icon' src='" + dfop.cpath + "/images/s.gif'/>");
            //checkbox
            if (dfop.showcheck && nd.showcheck) {
                if (nd.checkstate == null || nd.checkstate == undefined) {
                    nd.checkstate = 0;
                }
                ht.push("<img  id='", id, "_", nid, "_cb' class='bbit-tree-node-cb' src='", dfop.cpath + dfop.cbiconpath, dfop.icons[nd.checkstate], "'/>");
            }
            //a
        ...
        function check(item, state, type) {
            ...
            if (item.render && pstate != item.checkstate) {
                ...
                var et = $("#" + id + "_" + nid + "_cb");
                if (et.length == 1) {
                    et.attr("src", dfop.cpath + dfop.cbiconpath + dfop.icons[item.checkstate]);
                }
            }
        }
        //遍历子节点
