...
public class MethodCacheInterceptor implements MethodInterceptor,
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		if (element == null) {
			logger.debug("Find object from cache(key: " +cacheKey+ ") is null , Get method result and create cache........!");
			result = invocation.proceed();
			element = new Element(cacheKey, (Serializable)result);
			cache.put(element);
			logger.debug("create cache: " + cacheKey);
		} else {
			logger.debug("Find object from cache(key: " +cacheKey+ ") is not null");
		}
