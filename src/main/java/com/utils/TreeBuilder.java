...
public class TreeBuilder {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if( pid == rolef.getParentCode()){
				...
				if(!url.startsWith("http://")){
					url = "/" + rolef.getPriid() + "/index";
				}
				sb.append("{ \"" +tdConverter.getTreeAttrId()+ "\": \""+rolef.getPriid()+"\", \"" +tdConverter.getTreeAttrText()+ "\": \""+rolef.getPriname()+"\", ");
				sb.append("\"" +tdConverter.getTreeAttrCustomAttr()+ "\": {\"" +tdConverter.getTreeAttrUrl()+ "\": \""+ url + "\",\"" +tdConverter.getTreeAttrType()+ "\": \""+rolef.getType() + "\",\"" +tdConverter.getTreeAttrOrdernum()+ "\": \""+rolef.getOrdernum()+ "\"}");
	...
	private void bulidTree2(List<UsersResourse> list1,int pid) {
		/**
		 * 复制list对象，防止缓存为空
		 */
		List<UsersResourse> list = new ArrayList<UsersResourse>();
